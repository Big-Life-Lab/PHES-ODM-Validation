...
assets_path = "../assets/validation-rules/greater-than-max-length"
```
...
```{python}
invalid_dataset = import_dataset(assets_path + "/datasets/invalid-dataset.csv")
pprint_dict_list(invalid_dataset, "Invalid Dataset")
```
...
```{python}
valid_dataset = import_dataset(assets_path + "/datasets/valid-dataset.csv")
pprint_dict_list(valid_dataset, "Valid Dataset")
```
...
```{python}
error_report = import_json(assets_path + "/error-reports/error-report.json")
pprint(error_report)
```
...
pprint_dict_list(parts_v2, title = "Parts V2 Snippet")
```

The `phone` column in the `contacts` table would have this validation rule but the `addL2` column in the `addresses` table would not.
...
pprint_dict_list(parts_v1, title = "Parts Version 1")
```

The corresponding cerberus schema would be,

```{python}
schema_v2 = import_schema(assets_path + "/validation-schemas/schema-v1.yml")
pprint(schema_v2)
```
