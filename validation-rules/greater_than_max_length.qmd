...
```{python}
pprint_csv_file(assets_path + "/invalid-dataset.csv", "Invalid Dataset")
```
...
```{python}
pprint_csv_file(assets_path + "/valid-dataset.csv", "Valid Dataset")
```
...
```{python}
pprint_json_file(assets_path + "/error-report.json")
```
