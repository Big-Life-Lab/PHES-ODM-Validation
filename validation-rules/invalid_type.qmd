...
```{python}
pprint_csv_file(asset("datetime-invalid-dataset-1.csv"), "Invalid datetime dataset")
```
...
* **message**: Row \<row_index\> in column \<column_name\> in table \<table_name\> is a datetime with value \<invalid_value\> that has an unsupported datetime format. Allowed values are ISO 8601 standard full dates, full dates and times, or full dates and times with timezone.
...
```{python}
pprint_csv_file(asset("datetime-parts-v2.csv"), "Datetime parts v2")
```
...
```{python}
pprint_csv_file(asset("datetime-parts-v1.csv"), "Datetime parts v1")
```
