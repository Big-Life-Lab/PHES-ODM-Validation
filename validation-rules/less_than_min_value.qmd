...
This rule ensure that columns values are not less than a defined minimum value. Currently, this rule only supports columns whose data type is float or integer i.e. numbers. As an example, consider the `geoLat` column and `geoLong` column in the `sites` table. For the purpose of explaining this rule we'll assume the `geoLat` column is an `integer` with a minimum value of -90 and the `geoLong` column is a `float` with a minimum value of `-90.15`. The following dataset snippet would fail validation,
