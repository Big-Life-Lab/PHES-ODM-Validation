...
assets_path = "../assets/validation-rules/less-than-min-value"
```
...
If the value to be validated is not a number but can be coerced into one, then validation should proceed after coercion. Whether the corced value is valid or not, a warning should reported informing the user of the mismatched type. For example the following dataset would fail validation with an error and a warning,
...
If the value cannot be coerced into a number, then an error should be logged as in the following dataset where "a" cannot be coerced into a number

```{python}
pprint_csv_file(assets_path + "/invalid-dataset-3.csv", "Invalid Dataset")
```

The ODM dictionary is currently written so that the defined minimum values are inclusive i.e. only values less than the minimum values are invalid. All other values should pass this rule. For example, for the `geoLat` column, a value of -90 would be valid as in the dataset below,
...
1. The value is a number and is less than the min value
2. The value is not a number, can be coerced into one, and is less than the min value
...
The `geoLat` part which is a column in the `sites` table has a `minValue` value of -91 and would have this rule set for it. On the other hand, the `geoEPSG` part which is also a column in the `sites` table would not have this rule due its `minValue` value being `NA`.

## Cerberus Schema

Although the cerberus library supports [minimum value validation](https://docs.python-cerberus.org/en/stable/validation-rules.html#min-max), it does not support the less than **or** equal to operation. We will need to [extend the cerberus validator](https://docs.python-cerberus.org/en/stable/customize.html) to support this with a new `minExclusive` rule.
...
The metadata for this rule should include the row from the ODM for the part with its `partID` and `minValue` column values.
