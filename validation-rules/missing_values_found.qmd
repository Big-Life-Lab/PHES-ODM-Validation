...
assets_path = "../assets/validation-rules/missing-values-found"
```
...
```{python}
pprint_csv_file(assets_path + "/datasets/invalid-dataset-1.csv", "Invalid Dataset")
```
...
```{python}
pprint_csv_file(assets_path + "/datasets/invalid-dataset-2.csv", "Invalid Dataset")
```

Finally, if the ODM dictionary considers values of `NA` to be missing, the following would fail validation,

```{python}
pprint_csv_file(assets_path + "/datasets/invalid-dataset-3.csv", "Invalid Dataset")
```

The following dataset snippet would pass validation

```{python}
pprint_csv_file(assets_path + "/datasets/valid-dataset.csv", "Valid Dataset")
```
...
```{python}
pprint_json_file(assets_path + "/error-reports/error-report-3.json")
```
...
```{python}
pprint_csv_file(assets_path + "/parts/parts-v2.csv", "Parts v2")
```
...
```{python}
pprint_csv_file(assets_path + "/parts/parts-v1.csv", "Parts v1")
```
