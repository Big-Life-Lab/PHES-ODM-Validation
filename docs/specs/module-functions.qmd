...
1. `schema`: The rules to validate the data against. This is a dictionary that contains a [cerberus](https://docs.python-cerberus.org/en/stable/) schema object, as well as the odm-version it's based on. The cerberus schema object should ideally be generated using the `generate_validation_schema` function.
...
2. `by`: Specifies what to summarize by. An additional level of detail
   will be added under each table when `column` or `row` is selected. This
   argument is optional and defaults to `table` (without any extra level of
   detail).

   - type: `enum (table, column, row)`

### Return

Returns a summarized version of `report`.

- type: A `ReportSummary` object with the following fields and methods:
    * `data_version`: string consisting of the version of the ODM data
    * `schema_version`: string consisting of the version of the validation schema used
    * `package_version`: string consisting of the version of the validation package used
    - `errors`: A Python dictionary where the keys are table names and
      the values are Python dictionaries where the keys are rule names and the
      values are integers representing the number of times the rule were
      triggered.
    - `warnings`: Same as above
    - `to_markdown`: A method returning string with the text/markdown representation of the
      summary.
...
summary = summarize_report(report, by=table)
assert summary.errors == {
    'addresses': {
        'duplicate_entries_found': 5,
        'greater_than_max_length': 3,
        'invalid_type': 2,
    }
}
...
1. `parts`: The ODM data dictionary [excel
sheet](https://github.com/Big-Life-Lab/PHES-ODM/tree/V2.0.0-rc.3/dictionary-tables) 'parts'.
...
2. `sets`: The ODM data dictionary [excel sheet](https://github.com/Big-Life-Lab/PHES-ODM/tree/V2.0.0-rc.3/dictionary-tables) 'sets'.
