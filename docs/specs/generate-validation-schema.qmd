...
```{python}
#| echo: false
ASSET_DIR = f'../../assets/specs/generate-validation-schema/missing-version-1-fields'
```
...
The PHES-ODM data structure is complex. Trying to manually create the validation schema for the `[validate_data](./module-functions.md#validate_data)` function can be an error prone and laborious process, especially if the user tries to encode all the validation rules. However, programmatic generation of a validation schema is possible due to the efforts of the PHES-ODM team in creating a machine-redable data dictionary for the data.
...
* warningType: The warning type. Should be set to `missing-version-1-fields`.
