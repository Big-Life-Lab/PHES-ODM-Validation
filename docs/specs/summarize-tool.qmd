...
- `--by=[(table, column, row)],...`
  ...
  A comma separated list/set of the keys to summarize by. Each key specified
  will add a separate summarization to the summary report, but the same key
  can't be repeated as this is a set. Specifying an empty set (with `--by=`)
  will create a summary report without any error summaries. Defaults to
  `table`.

  Example: `--by=table,column`
...
```{shell}
> python tools/summarize.py $REPORT --by=table,column,row --format=markdown
...
> python tools/summarize.py $REPORT --by=table,column,row --out=$SUMMARY
> cat $SUMMARY
