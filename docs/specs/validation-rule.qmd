...
### `get_schema_keys` and `gen_schema`
...
The `gen_schema` method takes the following 4 parameters:
...
2. The `gen_schema` method would return the list `[6]` for the `siteID` column 
and `None` otherwise
...
2. The `gen_schema` method would return the list `['integer', 'integer', 90]`  
...
value that the rule returned in their `gen_schema` method for the table-column
...
The functionality of `get_schema_keys` could be rolled into `gen_schema`, 
