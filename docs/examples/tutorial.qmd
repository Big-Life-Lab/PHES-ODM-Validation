...
`validate_data` can take multiple tables at once, so we include our sample table in a Python dictionary with the table name as the key. We are validating the `samples` table in this example.

```{python}
schema_file = join(ASSET_DIR, f"validation-schemas/schema-v{ODM_VERSION_STR}.yml")
