...
You can extent [ODM validation schemas](../../../assets/validation-schemas/) for you specific needs. In this tutorial, we show how you to use  `schema_additions` parameter in the [generate_validation_schema](../../specs/generate-validation-schema.md) function to generate new validation rules.

## Extending a validation schema

### Setting the scenario

As an example, consider the `healthReg` part in the dictionary which is described as *The health planning authority where the site or insititute is located*. `healthReg` is a `varchar` column in the `sites` table. The core ODM validation schema does not have requirements `healthReg` other than requireing a `varchar` data type. However, you can add more rules, such as a list of valid health care regions in your testing program. 
...
For demonstration, the schema only contains two columns of the in the `sites` table:
...
parts = import_csv_file("./parts.csv")
