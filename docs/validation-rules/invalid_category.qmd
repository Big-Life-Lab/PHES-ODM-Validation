...
4. Use the sets sheet and the value of the `mmaSet` column to identify the categories. For each categorical column, filter the sets sheet to only include those rows whose `set` column value is the same as the `mmaSet` value. 
5. In the filtered rows, the `part` column contains the category values.
