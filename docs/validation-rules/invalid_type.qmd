...
3. [Get the defined data type for each column](../specs/odm-how-tos.md#getting-the-data-type-for-a-column). If the data type is a boolean then retreive the category values by using the sets sheet:
    1. Filter out all rows except for the ones whose `set` is `booleanSet`
    2. The `part` columns in the filtered rows contains the category values
...
The meta for this rule should include the `part`, `<table_name>`, and `dataType`.

For a boolean data type, it should also include the rows that contain the category values. The `part` and `set` from each row should be included in the category value rows.
