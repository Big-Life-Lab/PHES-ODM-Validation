...
An email address. As an example, imagine a part with id `contactEmail` which is
a header in a `labs` table and has its defined type to be `email`. The
following ODM snippet would fail validation,
...
[
    {
        ...
        "message": "Value john.doe in row 1 in column contactEmail in table
                    labs has type varchar but should be of type email or
                    coercable into an email"
    }
]
```
...
For an `email` type we will need to create a [custom type](https://docs.python-cerberus.org/en/stable/customize.html#new-types)
in cerberus.
