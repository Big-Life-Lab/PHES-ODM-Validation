...
Lets start by showing how to validate an ODM dataset. We will be working with a subset of the ODM dictionary. The dictionary has a sites table which "contains information about a site; the location where an environmental sample was taken.". The table has a number of columns but we'll be working with the `geoLat` and `geoLong` columns. These columns are mandatory in the table.
...
The above validation schema is actually based on a Python library called [cerberus](https://docs.python-cerberus.org/en/stable/schemas.html). Cerberus does all the validation heavy lifting in the package, the package is more a wrapper around it, making sure that the inputs and outputs to the package integrate well with the ODM. 
